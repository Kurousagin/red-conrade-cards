...
List<int> _shuffle(List<int> tiles) {
  ...
  for (int i = 0; i < 150; i++) {
    ...
    final tmp = arr[blankIdx]; arr[blankIdx] = arr[target]; arr[target] = tmp;
    blankIdx = target;
  }
  return arr;
}
...
class _PuzzleGameState extends State<PuzzleGame> {
  ...
  void _tap(int i) {
    ...
    if (!adjacent) return;
    setState(() {
      final tmp = _tiles[i]; _tiles[i] = _tiles[blankIdx]; _tiles[blankIdx] = tmp;
      _moves++;
      if (_tiles.asMap().entries.every((e) => e.value == [1,2,3,4,5,6,7,8,0][e.key])) {
        _won = true;
      }
    });
  }
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      _backBtn(),
                      const Expanded(child: Text('☭ QUEBRA-CABEÇA', textAlign: TextAlign.center,
                        style: TextStyle(fontFamily: 'Oswald', fontWeight: FontWeight.w700, fontSize: 14, color: SC.gold, letterSpacing: 1))),
                      Text('🎯 $_moves', style: const TextStyle(fontFamily: 'Oswald', fontSize: 14, color: SC.red, fontWeight: FontWeight.w700)),
                    ],
                  ),
                ),
                const SizedBox(height: 8),
                const Text('Deslize as peças para ordenar 1–8', style: TextStyle(fontSize: 12, color: SC.grey)),
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              color: val == 0 ? Colors.transparent : Color.lerp(SC.redDark, const Color(0xFF000044), val / 8),
                              border: Border.all(
                                color: val == 0 ? const Color(0xFF330000) : SC.red,
                                width: val == 0 ? 1 : 2,
                              ),
                              borderRadius: BorderRadius.circular(10),
                            ),
                            child: val == 0 ? null : Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Text(_puzzleSymbols[val], style: const TextStyle(fontSize: 22)),
                                Text('$val', style: const TextStyle(
                                  fontFamily: 'Oswald', fontWeight: FontWeight.w700, fontSize: 14, color: SC.gold,
                                )),
                              ],
                            ),
                          ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: const Text('🔄 EMBARALHAR', style: TextStyle(
                      fontFamily: 'Oswald', fontWeight: FontWeight.w700, fontSize: 13, color: SC.red,
                    )),
                  ),
                ),
              ],
